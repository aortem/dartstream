...
import 'package:ds_middleware/src/model/ds_response_model.dart';
...
class DsBodyParser {
  ...
  Future<DsCustomMiddleWareResponse> handleFileUpload(DsCustomMiddleWareRequest request) async {
  final contentTypeHeader = request.headers['content-type'];
  if (contentTypeHeader == null || !contentTypeHeader.contains('multipart/form-data')) {
    return DsCustomMiddleWareResponse.badRequest('Content-Type must be multipart/form-data');
  }

  final mediaType = MediaType.parse(contentTypeHeader);
  final boundary = mediaType.parameters['boundary'];
  if (boundary == null) return DsCustomMiddleWareResponse.badRequest('Missing boundary');
