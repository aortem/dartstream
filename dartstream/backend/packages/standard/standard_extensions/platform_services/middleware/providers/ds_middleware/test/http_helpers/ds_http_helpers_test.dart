...
void main() {
  group('DsHttpHelpers', () {
    ...
    test('error() returns standardized error', () {
      final response = DsHttpHelpers.error(message: 'DB failed', statusCode: 500);
    ...
    test('withCors() adds CORS headers', () {
      ...
      expect(corsResponse.headers['Access-Control-Allow-Methods'], 'GET, POST, PUT, DELETE');
    });

    test('withCache() adds Cache-Control header', () {
      final response = DsHttpHelpers.ok('Cache Test');
      final cacheResponse = DsHttpHelpers.withCache(response, maxAgeSeconds: 600, isPrivate: true);
  ...
  expect(response.statusCode, 200);
  expect(response.headers['Content-Disposition'], 'attachment; filename="temp_test.txt"');

  // Cast body as Stream<List<int>>
  final stream = response.body as Stream<List<int>>;

  final contents = await stream.fold<List<int>>([], (prev, element) => prev..addAll(element));
