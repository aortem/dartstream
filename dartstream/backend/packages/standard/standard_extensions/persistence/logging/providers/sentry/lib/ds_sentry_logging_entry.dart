...
import 'ds_sentry_logging_provider.dart';
...
void registerSentryLoggingProvider(Map<String, dynamic> config) {
  final provider = DSSentryLoggingProvider();

  DSLoggingManager.registerProvider(
    _providerName(config, 'sentry'),
    provider,
    DSLoggingProviderMetadata(
      type: 'sentry',
      region: _optionalString(config, 'region'),
      additionalMetadata: {
        if (config['dsn'] != null) 'dsn': config['dsn'],
      },
    ),
  );
}
