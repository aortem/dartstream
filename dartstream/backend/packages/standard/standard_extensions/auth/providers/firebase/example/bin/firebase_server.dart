// Firebase Authentication Sample Server
import 'dart:convert';
import 'dart:io';
import 'dart:math';
...
/// Dev mode flag - set to false to use real Firebase
...
Future<Response> handleRegister(Request request) async {
  try {
    ...
    print('📝 Simulating account creation for $email');
...
Future<Response> handleLogin(Request request) async {
  try {
    ...
    final userId = 'dev-${email.hashCode.abs()}';
...
Response handleHealth(Request request) {
  return Response.ok(
    jsonEncode({'status': 'ok', 'service': 'firebase-auth-demo', 'mode': 'development'}),
    headers: {'content-type': 'application/json'},
  );
}
...
Future<void> main() async {
  print('🚀 Starting Firebase Authentication Demo Server');
  ...
  print('\n💡 Open http://localhost:8080 in your browser to test the app');
}
